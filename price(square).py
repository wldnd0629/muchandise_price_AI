...
x_test_set = xz[0:50,1:-1]
y_test_set = xz[0:50,[-1]]
...
W1 = tf.get_variable("W1",shape = [3,3],initializer = tf.contrib.layers.xavier_initializer())
...
W2 = tf.get_variable("W2",shape = [3,nb_classes],initializer = tf.contrib.layers.xavier_initializer())
...
W5 = tf.get_variable("W5",shape = [nb_classes,1],initializer = tf.contrib.layers.xavier_initializer())
...
predicted1 = tf.argmax(hypothesis, 1)
...
with tf.Session() as sess:
    ...
    for step in range(1500): #in은 안에 머가 있느냐 라는 물음
    ...
    pred = sess.run(predicted1, feed_dict={X: x_test_set})
